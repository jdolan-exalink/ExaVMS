...
const RefreshIcon = (props: React.ComponentProps<'svg'>) => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2"/></svg>;
                    ...
                    <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                        ...
                        <div>
                            ...
                            <select name="auth.type" id="auth.type" value={formData.auth?.type || 'none'} onChange={handleChange} className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm bg-gray-50 dark:bg-gray-700 focus:border-primary-500 focus:ring-primary-500">
                                ...
                                <option value="frigate">Frigate Login (Cookie-based)</option>
                            </select>
                        </div>
...
const ServersTab = () => {
    ...
    const handleTestConnection = async (server: Server) => {
        console.log(`\n========== TESTING CONNECTION: ${server.name} ==========`);
        ...
        try {
            ...
            console.log(`========== TEST COMPLETE: ${server.name} - ${status.status.toUpperCase()} ==========\n`);
        } catch (error) {
            console.error(`========== TEST FAILED: ${server.name} ==========`, error);
            setStatuses(prev => ({ 
                ...prev, 
                [server.id]: { status: 'offline', resources: { cpu: 0, mem: 0, hdd: 0, gpu: 0 } }
            }));
        } finally {
                            ...
                            <tr key={server.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td className="py-4 px-6 text-right space-x-2">
                                    <button 
                                        ...
                                        className={`p-2 rounded-md hover:bg-blue-100 dark:hover:bg-blue-900/50 text-blue-500 ${testingServers.has(server.id) ? 'opacity-50 cursor-not-allowed animate-spin' : ''}`}
                                        title="Test Connection"
                                    >
                                        <RefreshIcon className="w-5 h-5"/>
                                    </button>
                                    <button onClick={() => handleOpenModal(server)} className="p-2 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-500 dark:text-gray-400" title="Edit"><EditIcon className="w-5 h-5"/></button>
                                    <button onClick={() => handleDelete(server.id)} className="p-2 rounded-md hover:bg-red-100 dark:hover:bg-red-900/50 text-red-500" title="Delete"><TrashIcon className="w-5 h-5"/></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
